...
> 1차 제출 피드백을 반영해 구조를 다시 잡았습니다. 처음엔 "독립된 가로 스크롤 하이재킹 두 개"로 이해했는데, 정확히는 **하나의 하이재킹 안에서 여러 섹션(카드 그룹)이 끊김 없이 이어지는 구조**([섹션1 카드들] → [섹션2 카드들] → ...)였습니다. 이에 맞춰 데이터/마크업/스크롤 로직을 전부 다시 짰고, 1차 제출에 있던 **Vanilla JS 구현은 코드베이스에서 완전히 제거**하고 GSAP ScrollTrigger 하나로만 구현했습니다.
...
[가로 스크롤 하이재킹 시작]
  ...
  → 섹션 4(섹션 추가 사례, 카드 3장) → 섹션 5(카드 추가 사례, 카드 15장)
  — 하이재킹이 풀리지 않고 이어서 흐름
[가로 스크롤 하이재킹 끝]
[더미 섹션: Outro]
```

섹션 4·5는 "섹션 수/카드 수가 늘어나도 레이아웃이 깨지지 않아야 한다"(요구사항 8)를 실제로 검증하려고 추가한 섹션입니다 — 섹션 개수를 3개에서 5개로, 카드가 가장 많은 섹션을 15장까지 늘려도 트랙 폭·스크롤 거리·타이틀 고정이 전부 데이터 배열 길이 기준으로 자동 재계산됩니다.
...
- **GSAP(ScrollTrigger)를 선택한 이유**: 이 과제의 핵심 난이도는 "세로 스크롤 델타를 가로 이동으로 변환"하는 계산 자체가 아니라, ① 트리거를 스크롤 위치가 아니라 화면 세로 "중앙"으로 잡고 ② 하나의 pin 안에서 여러 섹션이 경계 없이 이어지고 ③ 리사이즈/회전 시 스크롤 총 거리·트리거 지점이 자동으로 재계산되는 세 가지를 동시에 정확히 맞추는 데 있었습니다. 이건 직접 만들면 `IntersectionObserver`나 `wheel` 이벤트만으로는 처리하기 까다로운, 이미 GSAP ScrollTrigger의 `pin`/`scrub`/`invalidateOnRefresh` 옵션이 정확히 이 세 가지를 겨냥해 제공하는 기능입니다. ScrollMagic 같은 대안은 유지보수가 끊겼고, Locomotive Scroll류는 전체 페이지 스무스 스크롤에 초점이 맞아 있어 섹션 단위의 정밀한 pin 제어에는 GSAP만큼 맞지 않았습니다. 참고 사이트(sindoh.com)도 실제로 GSAP ScrollTrigger + Lenis 조합을 쓰고 있어, 이 종류의 인터랙션에 널리 검증된 선택이라고 판단했습니다.
- **Vanilla 구현을 완전히 제거한 이유**: 1차 제출은 Vanilla/GSAP 두 벌을 만들어 비교하는 방향이었는데, 구조를 다시 잡으면서(하나의 pin 안에 여러 섹션이 끊김 없이 이어지는 방식) 스크롤 진행률·트리거 계산이 한층 복잡해졌습니다. 이 복잡도를 `wheel` 이벤트를 직접 가로채는 Vanilla 방식으로 감당하는 것보다 GSAP 기능 위에서 정확성에 집중하는 편이 이번 재작업의 우선순위(정확한 구조로 확실하게 만드는 것)에 더 맞다고 판단해서, 비교용으로도 남기지 않고 코드베이스에서 완전히 들어냈습니다.
...
- 카드는 `<li>`가 아니라 `<a>`(포커스 가능, `aria-label`로 스크린리더 라벨 제공)로 만들어서 최소한 키보드 탭 순서에는 들어오도록 했습니다. 다만 카드 이동이 네이티브 가로 스크롤이 아니라 GSAP가 `scrollY`를 읽어서 매 프레임 `translateX`로 트랙을 미는 방식이라, 탭으로 포커스가 화면 밖 카드로 넘어갈 때 브라우저의 기본 "포커스 요소로 스크롤" 동작이 이 transform 기반 위치를 제대로 못 따라올 가능성이 있습니다(네이티브 `scrollLeft`가 아니라서). 이 동기화를 확실히 맞추려면 포커스 이벤트에서 해당 카드가 보이는 데 필요한 `scrollY`를 역산해 직접 `window.scrollTo`로 옮겨주는 로직이 필요한데, 시간 관계상 넣지 못했습니다.
