...
### 섹션 간격이 Chrome/Safari에서 다르게 보이던 문제

`.h-scroll-section`이 `flex-direction: column` 안에 텍스트(헤더)와 고정폭 카드 로우를 같이 담고 있으면, 이 박스의 "내용에 맞춰 줄어드는 폭(shrink-to-fit)"을 브라우저마다 다르게 계산합니다 — Chrome은 카드 로우 폭 그대로 딱 맞게 계산하는데 Safari는 여기에 조금 더 여유를 두면서 섹션 사이에 의도치 않은 간격이 생겼습니다. 카드 로우의 실제 렌더링 폭(`cardsEl.scrollWidth`)을 재서 섹션 박스 폭에 그대로 못박아버리는 방식(`el.style.width = ...`)으로 이 계산 자체를 브라우저에 맡기지 않도록 고쳤습니다. 리사이즈 시에도 어긋나지 않도록 `ScrollTrigger`의 `onRefreshInit`마다 다시 재고 다시 고정합니다.
