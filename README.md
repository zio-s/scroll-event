...
- **반응형 대응**: 1024px 이하에서는 스크롤 하이재킹을 유지하지 않고 `overflow-x: auto` + `scroll-snap-type: x proximity` 기반 네이티브 가로 스와이프로 바꿨습니다. 터치로 하이재킹을 구현하려면 touch 이벤트를 따로 가로채야 하는데 iOS Safari 등에서 스크롤이 끊기는 문제가 생기기 쉽고, 애초에 모바일에서는 스와이프가 더 자연스러운 인터랙션이라고 판단했습니다. 마우스로 테스트하는 경우(브라우저 창을 좁혀서 보는 경우 등)를 위해 클릭+드래그 스크롤(`dragScroll.ts`)도 추가했는데, 이건 터치의 네이티브 스크롤에는 관여하지 않고 `pointerType === 'mouse'`일 때만 동작합니다.
...
- 트랙패드/휠 입력의 관성감을 브라우저별로 더 세밀하게 맞추고 싶었습니다. 지금은 두 구현 모두 즉시 반응(scrub 없이 1:1 매핑)인데, GSAP 쪽은 `scrub` 값을 조절해서 살짝 관성을 주는 것도 시도해볼 만합니다.
