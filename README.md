...
- **반응형 대응**: 1024px 이하에서는 스크롤 하이재킹을 유지하지 않고 `overflow-x: auto` + `scroll-snap-type: x proximity` 기반 네이티브 가로 스와이프로 바꿨습니다. 터치로 하이재킹을 구현하려면 touch 이벤트를 따로 가로채야 하는데 iOS Safari 등에서 스크롤이 끊기는 문제가 생기기 쉽고, 애초에 모바일에서는 스와이프가 더 자연스러운 인터랙션이라고 판단했습니다. 세로 리스트로 바꾸는 방안도 고려했지만, 요구사항 8번처럼 카드/섹션 수가 늘어나도 안 깨지는 구조를 지향하다 보니 세로로 나열하면 카드가 많아질수록 스크롤 길이가 과도하게 길어지고 전체를 훑어보기 불편해질 수 있다고 판단해 제외했습니다. 실제로 참고 사이트(sindoh.com) 소스를 받아서 확인해봐도 모바일에서는 하이재킹만 완전히 풀고(`overflow:visible; transform:none`) 카드 리스트 자체는 그대로 `overflow:auto` 가로 스와이프로 유지하는, 저희와 같은 패턴을 쓰고 있었습니다.
