...
- **반응형 대응**: 1024px 이하에서는 스크롤 하이재킹을 유지하지 않고 `overflow-x: auto` + `scroll-snap-type: x proximity` 기반 네이티브 가로 스와이프로 바꿨습니다. 터치로 하이재킹을 구현하려면 touch 이벤트를 따로 가로채야 하는데 iOS Safari 등에서 스크롤이 끊기는 문제가 생기기 쉽고, 애초에 모바일에서는 스와이프가 더 자연스러운 인터랙션이라고 판단했습니다. 카드가 많아질수록 세로 리스트는 스크롤 길이가 과도하게 길어지고 전체를 훑어보기 불편해질 수 있다고 판단해 제외했습니다.

  참고 사이트(sindoh.com) 소스를 받아서 실제로 확인해봤는데, 정확히 짚자면: 그 사이트의 "산업별 도입 사례" 섹션 안에는 "산업별 도입 사례"·"기업 소식" 두 개의 하위 그룹이 있고, PC에서는 이 두 그룹을 하나의 pin 안에서 이어서 가로로 페이징하다가, 모바일에서는 `flex-direction: column`으로 그룹 자체를 세로로 쌓습니다(그룹 단위로는 세로 나열이 맞습니다). 다만 그룹 **안의 카드 리스트 자체**는 PC·모바일 모두 `overflow:auto` 가로 스와이프로 동일하게 유지되는데, 이 안쪽 리스트 동작이 저희 카드 리스트와 같은 패턴입니다. 저희는 섹션당 카드 하나의 평평한 리스트라 "그룹" 개념이 없지만, 과제 요구사항이 원래 [더미]→[섹션]→[더미] 구조를 요구하기 때문에 섹션 A/B 자체가 이미 독립적으로 pin되고 문서 흐름상 세로로 나열되어 있어서, "여러 가로 스크롤 단위가 세로로 나열되는" 결과 자체는 참고 사이트와 동일하게 얻고 있습니다.
...
- Playwright 등으로 리사이즈/브레이크포인트 전환 시나리오를 회귀 테스트로 남기고 싶었으나 과제 범위상 수동 확인으로 대신했습니다.
