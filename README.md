...
실무였다면 유지보수 안정성 때문에 **GSAP ScrollTrigger**를 최종 선택으로 판단했을 것 같습니다. 다만 그 판단 근거를 코드로도 보여드리고 싶어서, 같은 인터랙션을 순수 JS로 직접 구현한 버전을 섹션 A로, GSAP 버전을 섹션 B로 페이지에 함께 담았습니다. 자세한 비교는 아래 "기술 선택 이유"에 있습니다.
